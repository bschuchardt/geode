...
import org.apache.geode.distributed.internal.membership.rapid.MembershipImpl;
...
public class MembershipLocatorImpl<ID extends MemberIdentifier> implements MembershipLocator<ID> {
  ...
  private final RapidSeedLocator<ID> seedLocator;
  ...
  public MembershipLocatorImpl(int port, InetAddress bindAddress,
    ...
    seedLocator =
        new RapidSeedLocator<>(bindAddress, config.getLocators(), locatorsAreCoordinators,
            config.isNetworkPartitionDetectionEnabled(),
            locatorStats, config.getSecurityUDPDHAlgo(), workingDirectory, locatorClient,
            objectSerializer,
            objectDeserializer);

    handler.addHandler(PeerLocatorRequest.class, seedLocator);
    handler.addHandler(FindCoordinatorRequest.class, seedLocator);
    handler.addHandler(GetViewRequest.class, seedLocator);
  }
  ...
  @VisibleForTesting
  public RapidSeedLocator<ID> getRapidSeedLocator() {
    return this.seedLocator;
  }
  ...
  public void setServices(final Services<ID> services) {
    seedLocator.setServices(services);
  }
