...
public class NIOMsgReader extends MsgReader {
  ...
  public NIOMsgReader(Connection conn, Version version, BufferPool bufferPool)
  ...
  protected void ensureCapacity(int bufferSize) {
    ...
    nioInputBuffer = bufferPool.acquireDirectReceiveBuffer(bufferSize);
    ...
    if (oldBuffer != null) {
      ...
      lastProcessedPosition = 0;
      bufferPool.releaseReceiveBuffer(oldBuffer);
    }
  }
  ...
  public void close() {
    ...
    if (tmp != null) {
      ...
      bufferPool.releaseReceiveBuffer(tmp);
    }
  }
}
