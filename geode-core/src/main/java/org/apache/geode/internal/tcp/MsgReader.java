...
public class MsgReader {
  ...
  MsgReader(Connection conn, BufferPool bufferPool, InputStream inputStream,
      Version version) {
  ...
  Header readHeader() throws IOException {
    ByteBuffer buffer = readAtLeast(Connection.MSG_HEADER_BYTES);

    Assert.assertTrue(buffer.remaining() >= Connection.MSG_HEADER_BYTES);

    int messageLength = buffer.getInt();
    Connection.throwExceptionIfWrongMessageVersion(messageLength);
