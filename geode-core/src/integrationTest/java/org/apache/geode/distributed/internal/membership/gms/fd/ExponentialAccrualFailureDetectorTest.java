...
public class ExponentialAccrualFailureDetectorTest {
  @Test
  public void test() {
    ExponentialAccrualFailureDetector failureDetector = new ExponentialAccrualFailureDetector(16, 200, 500);
    ...
    for (int i = 0; i < 300; i++) {
      ...
      if (i > 290) {
        ...
        System.out.println("for interval " + i + " lastHeartbeat="+failureDetector.getLastTimestampMillis()+", phi=" + phi);
        if (i == 291) {
  ...
  public void isAvailableTest() throws Exception {
    ...
    int heartbeatInterval = 2500;
    ExponentialAccrualFailureDetector detector =
        new ExponentialAccrualFailureDetector(threshold, historySize, heartbeatInterval);
